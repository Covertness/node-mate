...
function Network(options) {
	...
	this.udpNode.on('message', function (data, remote) {
		try {
			...
			console.log('Mate udp node reveive invalid message: decode error');
			return;
		}

		if (protocol.types[message.type] === undefined) {
			that.emit("data_error", 'unknown type');
			console.log('Mate udp node reveive invalid message: unknown type');
			return;
		}

		console.log('Mate udp node reveive message: %j', message);
...
Network.prototype.sendMessage = function (message, address) {
	console.log('Mate udp node send message: %j', message);

	var data = bencode.encode(message);

	// console.log('Mate udp node send message: ' + data.toString());
